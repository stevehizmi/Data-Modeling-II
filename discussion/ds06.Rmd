...
ratings.standard.us = ratings.standard %>%
  filter(Country == "United States" & B_day < 2000)

ggplot(ratings.standard.us, aes(x= Rating , color=Sex)) + geom_density() +
   ggtitle("Rating of female versus male chess players")

# don't forget to add appropriate title/axes!!
```
The ratings aren't much different, as the male and female curves stay consistent with each other across the graph.They are more similar than they are different.
...
ratings.standard.us %>%
  ...
  summarise(num  = n() ,  # for help, see dplyr.tidyverse.org/reference/n.html
            ...
            max  = max(Rating) )
```
205 / 4,883 = 4.2% of non-junior American players are women.The top rated female player is 394 points below the top rated male player.
...
true_diff = 394
# # run the function
diffs = permutedMaxSexDiff(ratings.standard.us,10000)
...
pval = mean(diffs > true_diff)
pval
```

> _**My results are not consistent with Ma's results. My p value is 0.008, meaning that the null hypothesis should be reject. This does not mean, however, that there aren't some other factors other than participation level that can explain the gap between male and female ratings.**_
...
permutedCoords = function(df,n){
  ...
  for(i in 1:n){
    # permute ratings
    permuted = sample(df$drat)
    
    # run linear regression
    reg = lm(mpg ~ permuted, data=df)$coefficients[8]
...
perm = permutedCoords(mtcars.small, 1000)
